...
def graph_compile(target: Union[str, None] = "tf", **compile_kwargs):
    if target is None:
        return target

    target = target.lower()
    if target != "tf" and target != "xla":
        raise ValueError(f"Unknown compile target '{target}'. Choose either 'tf' or 'xla'")

    if target == "xla":
        compile_kwargs["jit_compile"] = True

    def _decor_fn(func):
        return tf.function(func, **compile_kwargs)
...
class Scipy:
        ...
        **scipy_kwargs,
        ...
        Args:
            ...
            step_callback: If not None, a callable that gets called once after
                ...
                step.
            compile: If "tf", wraps the evaluation function (the passed `closure`
                as well as its gradient computation) inside a `tf.function()`,
                which will improve optimization speed in most cases. If "xla" is
                used then `tf.function(jit_compile=True)` will be applied.
    ...
    @classmethod
    def eval_func(
        cls, closure: LossClosure, variables: Sequence[tf.Variable], compile: Optional[str] = "tf"
        ...
        if compile is not None:
            _tf_eval = graph_compile(compile)(_tf_eval)
